...
const getCancionesByArtista = (req, res) => {
    ...
    conn.query("SELECT canciones.id, cancion.nombre, artistas.id AS nombre_artista FROM canciones INNER JOIN albumes ON albumes.id = canciones.album  INNER JOIN artistas ON artistas.id = albumes.artista WHERE artistas.id = ?",[getCancionesByArtista],  (err, rows) => 
